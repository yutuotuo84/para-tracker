...
async def list_tags(db: AsyncSession = Depends(get_session)):
    """获取 PARA 标签树"""
    tree = await get_tag_tree(db)
    return tree


@router.post("/tags")
async def add_tag(req: CreateTagRequest, db: AsyncSession = Depends(get_session)):
    """创建新标签"""
    if not req.full_path or req.full_path.strip() == "":
        raise HTTPException(status_code=400, detail="标签路径不能为空")

    tag = await create_tag(db, req.full_path.strip(), req.label)
    return {
        "id": tag.id,
...
async def edit_tag(tag_id: int, req: UpdateTagRequest, db: AsyncSession = Depends(get_session)):
    """修改标签名称"""
